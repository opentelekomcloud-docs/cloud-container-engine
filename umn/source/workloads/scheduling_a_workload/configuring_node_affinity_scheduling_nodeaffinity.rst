...
#. In **Advanced Settings**, choose **Scheduling** and select a policy for **Node Affinity**. In this example, **Customize affinity** is selected. For details about how to create a workload, see :ref:`Creating a Workload <cce_10_0673>`.
   ...
   .. table:: **Table 1** Scheduling policies
      ...
      |                       | -  **Specify node**: You can specify the node where the pods will be deployed. The system selects nodes based on the **kubernetes.io/hostname** label. Each node has a unique label value. If no nodes are specified, the pods will be randomly scheduled according to the default cluster scheduling policy.                                                                         |                       |
      |                       | -  **Specify node pool**: You can specify the node pool where pods will be deployed. The system selects nodes based on the **cce.cloud.com/cce-nodepool** label. Nodes in the same node pool have the same label value, allowing any of them to run the pods. If no node pools are specified, the pods will be randomly scheduled according to the default cluster scheduling policy. |                       |
...
#. In the dialog box that is displayed on the right, click **Add Policy** to configure rules for filtering node labels.
   ...
   .. table:: **Table 3** Parameters for configuring node affinity scheduling policies
      ...
      |                       |                                                                                                                                                                                                                                                                                                                                                      |                       |
      |                       | Both default labels and custom labels are supported. For details about default node labels, see :ref:`Inherent Labels of a Node <cce_10_0004__section74111324152813>`.                                                                                                                                                                               |                       |
      +-----------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-----------------------+
...
Workload node affinity rules are implemented using node labels. When a node is created in a CCE cluster, it is automatically assigned certain labels. Here are some examples of commonly used node labels (for more labels, see :ref:`Inherent Labels of a Node <cce_10_0004__section74111324152813>`):
...
.. |image1| image:: /_static/images/en-us_image_0000002483959384.png
